...
export default function ItineraryPage() {
  const { data: trips, isLoading: tripsLoading } =
    api.trips.getItineraries.useQuery();
  ...
  return (
    <div className="horizon-bg relative min-h-screen">
      <main className="mx-auto flex w-full max-w-6xl gap-6 px-6 py-12">
        <Card className="w-72 self-start rounded-2xl border border-white/40 bg-white/80 shadow-xl backdrop-blur-sm">
          <CardHeader className="text-lg font-semibold text-[#0A2A43]">
            Your Trips
          </CardHeader>
            ...
            {tripsLoading ? (
              ...
              trips?.map((trip) => (
                <Button
                  ...
                  className="mt-2 flex w-full items-center justify-between rounded-xl bg-white/90 text-sm hover:bg-[#0A2A43]/5"
                  variant="outline"
                >
        ...
        <AddItinerary
          ...
          setOpenAddTrip={setOpenAddTrip}
        />
        <JoinTrip openJoin={openJoin} setOpenJoin={setOpenJoin} />

        {selectedTrip ? (
          <Card className="flex-1 rounded-2xl border border-white/40 bg-white/80 shadow-xl backdrop-blur-sm">
            <CardContent className="p-8">
              <GetDays
                itineraryId={selectedTrip}
                onTripDeleted={() => setSelectedTrip(null)}
              />
            </CardContent>
          </Card>
        ) : (
          <Card className="flex flex-1 items-center justify-center rounded-2xl border border-white/40 bg-white/80 shadow-xl backdrop-blur-sm">
            <CardContent className="max-w-md px-4 py-10 text-center">
              <ChevronLeftIcon className="text-muted-foreground mx-auto mb-4 h-16 w-16" />
              <h2 className="mb-2 text-2xl font-semibold text-[#0A2A43]">
                No Trip Selected
              </h2>
              <p className="text-muted-foreground text-sm">
                Select a trip from the sidebar to view and manage your
                itinerary, or create a new trip to get started on your next
                adventure.
              </p>
            </CardContent>
          </Card>
...
function GetDays({
  ...
  const { data: days, isLoading: daysLoading } = api.trips.getDays.useQuery({
    itineraryId,
  });
  const { user } = useAuth();
  ...
  const collaborators = (itinerary?.collaborators ?? []).filter(
    (c) => !!c.profile,
  );
  ...
  const [editDayNumber, setEditDayNumber] = useState<number | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "presence",
        { event: "join" },
        (payload: { newPresences?: PresenceEntry[] }) => {
      ...
      .on(
        "presence",
        { event: "leave" },
        (payload: { leftPresences?: PresenceEntry[] }) => {
          const ids = (payload.leftPresences ?? []).map(
            (p) => p.userId ?? p.id,
          );
    ...
    <div className="flex flex-col items-center gap-6">
      <div className="max-w-2xl text-center">
        <div className="flex flex-row items-center justify-center gap-2">
          <h1 className="flex-1 text-3xl font-black text-[#0A2A43] md:text-4xl">
            {itinerary?.title}
          </h1>
        ...
        <p className="pt-3 text-base text-gray-700">{itinerary?.description}</p>
      </div>

      <Card className="w-full max-w-3xl rounded-2xl border border-slate-100 bg-white/90 shadow-md dark:border-slate-700 dark:bg-slate-900/80">
      ...
      <div className="w-full max-w-3xl">
        {daysLoading ? (
          ...
          days?.map((day) => (
            <Card
              className="mb-3 rounded-2xl border border-slate-100 bg-white/90 shadow-sm dark:border-slate-700 dark:bg-slate-900/80"
              key={day.id}
            >
              <CardHeader>
                <div className="flex flex-row items-center justify-between">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="rounded-full p-1 hover:bg-slate-100">
                        <Ellipsis className="text-muted-foreground h-5 w-5 cursor-pointer" />
                      </button>
                    </DropdownMenuTrigger>
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        ...
        ({ new: a }: { new: Activity }) => {
          utils.trips.getActivities.setData(
            { itineraryDayId: dayId },
            (old) => {
              if (!old) return [a];
    ...
    <div className="mt-3">
      {activitiesLoading ? (
        ...
        sortedActivities.map((activity) => (
          <Card
            className="mb-2 space-y-2 rounded-xl border border-slate-100 bg-white/90 shadow-sm dark:border-slate-700 dark:bg-slate-900/80"
            key={activity.id}
          >
            <CardHeader className="py-3">
              <div className="flex items-center justify-between">
                <div className="flex flex-row items-center gap-3">
                  ...
                  <Badge className="w-fit rounded-full border border-[#ffb88c]/40 bg-[#ffb88c]/20 px-2 py-0.5 text-xs font-semibold text-[#ffb88c]">
                    {activity.category}
                  </Badge>
                </div>
                <DropdownMenu>
            ...
            <CardContent className="flex flex-col gap-3 pt-0 pb-3">
